...
int nvme_open(nvme_root_t r, const char *name, nvme_link_t *lp)
{
	nvme_link_t l;
	int ret;
...
int nvme_get_telemetry_max(nvme_link_t l, enum nvme_telemetry_da *da, size_t *data_tx)
{
	...
	err = nvme_identify_ctrl(l, id_ctrl);
	if (err)
		return err;
			   ...
			   size_t *size)
{
	...
	*size = 0;
	...
	if (ctrl) {
		err = nvme_get_log_telemetry_ctrl(l, true, 0, log, xfer);
		...
		if (create)
			...
			err = nvme_get_log_telemetry_host(l, 0, log, xfer);
	}
	...
	*size = (dalb + 1) * xfer;
	...
	err = nvme_get_log(l, NVME_NSID_NONE, rae, NVME_LOG_LSP_NONE,
...
static int nvme_check_get_telemetry_log(nvme_link_t l, bool create, bool ctrl, bool rae,
					struct nvme_telemetry_log **log, enum nvme_telemetry_da da,
					size_t *size)
{
	enum nvme_telemetry_da max_da = 0;
	int err = nvme_get_telemetry_max(l, &max_da, NULL);
	...
	return nvme_get_telemetry_log(l, create, ctrl, rae, 4096, da, log, size);
}
...
{
	return nvme_check_get_telemetry_log(l, false, true, rae, log, da, size);
}

int nvme_get_host_telemetry(nvme_link_t l, struct nvme_telemetry_log **log,
		enum nvme_telemetry_da da, size_t *size)
{
	return nvme_check_get_telemetry_log(l, false, false, false, log, da, size);
}

int nvme_get_new_host_telemetry(nvme_link_t l, struct nvme_telemetry_log **log,
		enum nvme_telemetry_da da, size_t *size)
{
	return nvme_check_get_telemetry_log(l, true, false, false, log, da, size);
}
...
int nvme_get_lba_status_log(nvme_link_t l, bool rae, struct nvme_lba_status_log **log)
{
	...
	err = nvme_get_log_lba_status(l, true, 0, buf, sizeof(*buf));
	if (err) {
		*log = NULL;
		return err;
	}
...
static int __nvme_insert_tls_key_versioned(key_serial_t keyring_id, const char *key_type,
					   ...
					   long *key)
{
	...
	return nvme_update_key(keyring_id, key_type, identity,
			       psk, key_len, key);
}
...
static int __nvme_insert_tls_key(long keyring_id,
				 const char *hostnqn, const char *subsysnqn,
				 const char *identity, const char *key, long *keyp)
{
	...
	if (hmac == NVME_HMAC_ALG_NONE || !identity) {
		 ...
		 */
		ret = __nvme_insert_tls_key_versioned(keyring_id, "psk",
						      hostnqn, subsysnqn,
						      version, hmac,
						      key_data, key_len,
						      &lkey);
	} else {
		ret = nvme_update_key(keyring_id, "psk", identity,
				      key_data, key_len, &lkey);
	}
				   ...
				   long *keyring_id, long *key_id)
{
	...
	if (!id)
		ret = __nvme_insert_tls_key(kr_id, hostnqn,
